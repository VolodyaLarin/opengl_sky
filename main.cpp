...
//                discard;
            ...
            vec3 day_extinction = exp(-exp(-((pos.y + fsun.y * 4.0) * (exp(-pos.y * 16.0) + 0.1) / 80.0) / Br) *
                                      (exp(-pos.y * 16.0) + 0.1) * Kr / Br) * exp(-pos.y * exp(-pos.y * 8.0) * 4.0) *
                                  exp(-pos.y * 2.0) * 4.0;
            ...
            // Cumulus Clouds
            for (int i = 0; i < 3; i++) {
                float density = smoothstep(1.0 - cumulus, 1.0, fbm((0.7 +
                                                                    float(i)
                                                                    * 0.01) * pos.xyz / pos.y + time * 0.3));
...
const char *postFragShader = GLSL(
        ...
        void main() {
            ...
            for (int i = -2; i < 3; i++) {
                for (int j = -2; j < 3; j++) {
                    occlusion +=
                            1.0 / (1.0 + pow(10.0 * min(depth - texture(tex[1], UV + vec2(i, j) * r).r, 0.0), 2.0)) /
                            24.0;
                }
            }
            color.rgb *= occlusion;
...
typedef struct __attribute__((packed)) {
    char magic[2];
    unsigned int size, reserved, offset, hsize, width, height, colors, compression, image_size, h_res, v_res, palletes, colors2;
} bmp_header;
...
matrix getProjectionMatrix(int w, int h) {
    ...
    float far = 1000.0f;
    matrix m;
    m.m[0] = 1.0f / (aspect * tanf(fov * 3.14f / 180.0f / 2.0f));
    m.m[5] = 1.0f / tanf(fov * 3.14f / 180.0f / 2.0f);
...
entity makeEntity(scene *s, const char *vs, const char *fs, int texcount, char textures[][40],
                  ...
                  int w, int h) {
    entity e = {.vertices = vertices, .depth_test = depth_test, .texcount = texcount,};
    ...
    for (unsigned int i = 0; i < layouts; i++) {
        ...
        glVertexAttribPointer(i, 3, GL_FLOAT, GL_FALSE, (int) (sizeof(float) * layouts * 3),
    ...
    e.cirrus = glGetUniformLocation(e.program, "cirrus");
    e.cumulus = glGetUniformLocation(e.program, "cumulus");
...
void renderEntity(entity e, matrix P, matrix V, RenderParams params) {
    glUseProgram(e.program);
    for (int i = 0; i < (e.fb ? 2 : e.texcount); i++)
        glActiveTexture(GL_TEXTURE0 + (unsigned int) i), glBindTexture(GL_TEXTURE_2D, e.textures[i]), glUniform1i(
                e.tex + i, i);
    glUniformMatrix4fv(e.P, 1, GL_FALSE, P.m);
    glUniformMatrix4fv(e.V, 1, GL_FALSE, V.m);
    glUniform1f(e.time, params.time);
    glUniform1f(e.cirrus, params.cirrus);
    glUniform1f(e.cumulus, params.cumulus);
...
void renderScene(scene s, int w, int h) {
    ...
    for (unsigned int i = 0; i < s.entity_count; i++)
        if (!s.entities[i].fb)
            renderEntity(s.entities[i], p, v, s.params);
    for (unsigned int i = 0; i < s.entity_count; i++)
        if (s.entities[i].fb)
            renderEntity(s.entities[i], p, v, {0, s.params.cumulus, s.params.cirrus});
}
...
int main() {
    ...
    makeEntity(&s, postVertShader, postFragShader, 0, NULL, NULL, 4, 0, 1, 0, max_display_w, max_display_h);
